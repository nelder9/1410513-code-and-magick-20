...
var renderBars = function (ctx, players, times, maxTime) {
...
var renderPlayers = function (ctx, players, times, maxTime) {
  for (var i = 0; i < players.length; i++) {
    if (players[i] === 'Вы') {
...
window.renderStatistics = function (ctx, players, times) {
  ...
  renderPlayers(ctx, players, times, maxTime);
  renderBars(ctx, players, times, maxTime);
  renderHeader(ctx);
};
